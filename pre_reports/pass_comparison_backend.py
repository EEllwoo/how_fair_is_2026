...
def summarize_disagreements(comparison_dataframe):
    ...
    return summaryfrom pathlib import Path
...
def _load_pass_dataframe(csv_path):
    ...
    dataframe[PAPER_NAME_COLUMN] = dataframe[PAPER_NAME_COLUMN].map(_normalize_scalar)
    dataframe = dataframe.set_index(PAPER_NAME_COLUMN, drop=True)
    dataframe.index.name = PAPER_NAME_COLUMN
    return dataframe
...
def get_overlap_summary(first_pass_path=DEFAULT_FIRST_PASS, second_pass_path=DEFAULT_SECOND_PASS):
    first_pass, second_pass = load_pass_pair(first_pass_path, second_pass_path)
    first_titles = set(first_pass.index)
    second_titles = set(second_pass.index)
    matched_titles = sorted(first_titles & second_titles)

    return {
        "first_pass_papers": len(first_pass),
        "second_pass_papers": len(second_pass),
        "matched_papers": len(matched_titles),
        "first_only_papers": sorted(first_titles - second_titles),
        "second_only_papers": sorted(second_titles - first_titles),
    }


def get_comparison_fields(first_pass, second_pass, scope="all"):
    scope_name = scope.lower()
    shared_fields = [column for column in first_pass.columns if column in second_pass.columns]
...
def _paper_exists(first_pass, second_pass, paper_name):
    normalized_name = _normalize_scalar(paper_name)
    missing_from = []

    if normalized_name not in first_pass.index:
        missing_from.append(PASS_LABELS[0])
    if normalized_name not in second_pass.index:
        missing_from.append(PASS_LABELS[1])
...
def _build_comparison_rows(first_pass, second_pass, paper_name, fields, differences_only):
    rows = []

    for field in fields:
        first_value = _normalize_scalar(first_pass.at[paper_name, field]) if field in first_pass.columns else ""
        second_value = _normalize_scalar(second_pass.at[paper_name, field]) if field in second_pass.columns else ""
        matches = first_value == second_value
...
def compare_paper_marks(
    ...
    first_pass, second_pass = load_pass_pair(first_pass_path, second_pass_path)
    normalized_name = _paper_exists(first_pass, second_pass, paper_name)
    fields = get_comparison_fields(first_pass, second_pass, scope=scope)
...
def compare_all_papers(
    ...
    first_pass, second_pass = load_pass_pair(first_pass_path, second_pass_path)
    matched_papers = sorted(set(first_pass.index) & set(second_pass.index))
    fields = get_comparison_fields(first_pass, second_pass, scope=scope)

    rows = []
    for paper_name in matched_papers:
        rows.extend(
            _build_comparison_rows(
                first_pass=first_pass,
                second_pass=second_pass,
